...
class TerminalViewListDriver
    ...
    def find
        ...
        case answer
        when 1
            puts self.list_driver.find(:fullname,want_to_find) == nil ? "Такого водителя нет!" : self.list_driver.find(:fullname,want_to_find)
        when 2
            puts self.list_driver.find(:passport,want_to_find) == nil ? "Такого водителя нет!" : self.list_driver.find(:passport,want_to_find)
        when 3
            puts self.list_driver.find(:mobphone,want_to_find) == nil ? "Такого водителя нет!" : self.list_driver.find(:mobphone,want_to_find)      
        end
    ...
    def change
        ...
        driver = self.list_driver.find(:passport,want_to_change)
        if driver == nil
        ...
        array_changes.each do |item|
            ...
            self.list_driver.change(driver,fields[item],change)
        end
    ...
    def delete
        ...
        driver = self.list_driver.find(:passport,want_to_delete)
        if driver == nil
            puts "Такого водителя нет!"
        end
        self.list_driver.delete(driver)
    end
    ...
    def sort
        ...
        self.list_driver.sort(fields[answer])  
    end
